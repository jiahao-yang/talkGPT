...
def transcribe_speech(language):
    ...
    # concatenate the audio chunks into a single audio object
    audio_data = b''.join([chunk.get_raw_data(convert_rate=16000, convert_width=2) for chunk in audio_chunks])
    audio = sr.AudioData(audio_data, 16000, 2)
    try:
        # transcribe the concatenated audio to text
